...
// There is a clever optimization that reduces the cost to only one bit for each
// pair of blocks. This single bit is B1_is_free XOR B2_is_free, for a buddy pair
// of blocks B1 and B2. Each time a block is allocated or freed, then flip the bit
// to reflect the change.
void
bit_flip(char *array, int index)
{
  // one bit for each pair of blocks.
  index >>= 1;
  // char b = array[index / 8];    // Find the byte.
  char m = (1 << (index % 8));  // Got the position.
  array[index / 8] ^= m;        // Make the bit XOR 1;
}

// If the bit in index of array was set.
int
bit_isset_pair(char *array, int index)
{
  index >>= 1;
  char b = array[index / 8];
  char m = (1 << (index % 8));
...
bd_initfree_pair(int k, int bi, int flag) {
  ...
  */
  ...
  if(bit_isset_pair(bd_sizes[k].alloc, bi)){
    // one of the pair is free.
    free = BLK_SIZE(k);
    if(flag){
